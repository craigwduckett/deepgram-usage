...
export default function RequestDetails({ request, onBack }: RequestDetailsProps) {
  ...
  const hasPathComponents = parsedPath.segments.length > 0 || Object.keys(parsedPath.queryParams).length > 0;
                ...
                <div className="grid grid-cols-3 gap-1">
                  ...
                  <dd className="col-span-2">
                    ...
                    <span className="ml-2 text-xs text-muted-foreground">
                      ({new Date(request.created).toISOString().replace('T', ' ').substring(0, 19)} UTC)
                    </span>
                  </dd>
                </div>
                <div className="grid grid-cols-3 gap-1">
                ...
                <div className="grid grid-cols-3 gap-1">
                  ...
                  <dd className="col-span-2">
                    <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${
                      ...
                      {request.response?.code || request.code}
                    </span>
                  </dd>
                </div>
                <div className="grid grid-cols-3 gap-1">
              ...
              <div className="mt-6">
                ...
                <div className="rounded-lg border bg-card p-4">
                  {parsedPath.segments.length > 0 && (
                    <>
                      <h4 className="text-xs font-medium text-muted-foreground mb-2">Path Segments:</h4>
                      <ul className="mb-3 space-y-1">
                        {parsedPath.segments.map((segment, index) => (
                          <li key={index} className="text-xs bg-muted rounded px-2 py-1 break-all">
                            <span className="font-mono">{index + 1}:</span> {segment}
                          </li>
                        ))}
                      </ul>
                    </>
                  ...
                  {Object.keys(parsedPath.queryParams).length > 0 && (
                    <>
                      ...
                      <dl className="space-y-1">
                        {Object.entries(parsedPath.queryParams).map(([key, value], index) => (
                          <div key={index} className="grid grid-cols-3 gap-1 text-xs">
                            <dt className="font-mono break-all">{key}:</dt>
                            <dd className="col-span-2 bg-muted rounded px-2 py-1 break-all">{decodeURIComponent(value || '')}</dd>
                          </div>
                        ))}
                      </dl>
                    </>
